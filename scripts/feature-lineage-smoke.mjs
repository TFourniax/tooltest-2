...
const root=process.cwd(),temp=fs.mkdtempSync(path.join(os.tmpdir(),'idleproof-lineage-'));
...
try {
  ...
  assert.equal(result.links[0].fromObservation,state.features[old.featureKey].lineageObservations.items[0].id);
  ...
  assert.deepEqual(fs.readFileSync(statePath),stateBytes);assert.deepEqual(fs.readFileSync(path.join(repo,'.git','index')),index);
  console.log(JSON.stringify({schema:'idleproof-feature-lineage-smoke-1',classification:'MACHINE',passed:true,
    ...
    noScoreTransfer:true,readOnly:true,corruptJournalRejected:true}));
} finally {

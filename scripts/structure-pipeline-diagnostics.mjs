...
try {
  ...
  // Exactly 100 fresh directories, independent of success/failure. Configuration
  // mode mirrors JSON/TOML/YAML; language mode starts with the exact TS task
  // and touched-file list from the Windows failure, then observes each language.
  for(let round=1;round<=100;round++) {
    const cwd=path.join(root,String(round));fs.mkdirSync(cwd);
    if(languageMode) for(const [file,source]of fixtures) fs.writeFileSync(path.join(cwd,file),source);
    for(const [file,source,provider,symbol,count]of fixtures) {
      if(!languageMode) fs.writeFileSync(path.join(cwd,file),source);
      for(let call=1;call<=count;call++) {
        ...
        try {
          const value=extractTaskSignals(cwd,{currentResource:file,prompt:'Inspect invented',
            ...(languageMode ? {currentCapabilities:['code.read'],touchedFiles:fixtures.map(([name])=>name)} : {})});
...
console.log(JSON.stringify({schema:'idleproof-source-pipeline-diagnostic-1',classification:'MACHINE',qualification:false,
  note:'Fixed 100 synthetic sequences; 500 configuration or 400 language observations. All outcomes retained. Instrumentation overhead is included. No retries, raw source/response, command, path, stderr or exception message. The original gate remains authoritative.',
  platform:process.platform,arch:process.arch,node:process.version,expectedSamples:process.argv.includes('--languages')?400:500}));

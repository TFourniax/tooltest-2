...
## Fresh Windows main failure, 2026-09-23

Idle main d375177a1a92442eead09f178b37e8957c4824d1 run35927891752,
attempt1: 15/16 jobs passed. Windows job107407243054 failed the first
JS/TS/Go/Rust task symbol assertion (invented instead of actual). The log
does not preserve the extraction rejection/process reason; timeout is not
an established cause. Core/grammar installation and preceding steps passed.

This revision preserves existing PR18 and qualified main19 work by merging
both histories. It adds a fixed 100-directory/400-observation Windows language
mode to the existing pipeline diagnostic. Each sequence starts with the exact
TypeScript task and four touched files from the original assertion, then
observes each remaining language. The authoritative smoke test, 500ms timeout
and 150ms hook p95 remain unchanged. Diagnostic and artifact outcomes are
retained independently and cannot override the original gate.

Local Linux mode: all400 observations canonical and expected-symbol matches,
zero timeouts, maximum elapsed411.137659ms. These are instrumented observations,
not a Windows correction or latency qualification. All records and their hash
are retained in evidence/PM_010B_pipeline_languages_linux.jsonl.gz.b64 and
PM_010B_pipeline_languages_local.json. Both historical incidents remain open.

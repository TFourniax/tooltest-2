...
function userMessage(explanation, phase) {
  const handoff=phase==='handoff';
  const lines=[
    `IdleProof · ${handoff?'task handoff':'what this means in your project'}`,
...
export function buildHookDelivery(cwd,state,session,eventName) {
  ...
  const signals=extractTaskSignals(cwd,session);
  const phase=detectLearningPhase({...session,taskSignals:signals});
  if (phase!=='handoff' && !signals.file) return null;
  const enriched={...session,taskSignals:signals};
  ...
  return {key,message:userMessage(explanation,phase),explanation,signals,phase};
}

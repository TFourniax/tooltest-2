...
Usage:
  idleproof on                 Install Claude Code hooks + open live dashboard
...
function doctor(cwd) {
  ...
  const required = checks.filter(([name]) => name !== 'Claude hooks');
...
export async function main(args) {
  ...
  if (command === 'install' && subcommand === 'claude') {
    ...
    console.log(`✓ Installed IdleProof hooks in ${file}`);
    console.log('Run `idleproof serve` to open the live learning surface.');
    return;
  }

  if (command === 'uninstall' && subcommand === 'claude') {
    const changed = uninstallClaude({ cwd });
    console.log(changed ? '✓ IdleProof hooks removed; other Claude settings were preserved.' : 'No IdleProof hooks found.');
    return;
  }
  ...
  if (command === 'reset') {
    ...
    console.log('✓ Local IdleProof state reset. Claude hooks were left untouched.');
    return;
  }
